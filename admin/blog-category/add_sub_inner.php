<?php
  $show = 'Sub Inner Category';
  include("../layout/header.php");
  extract($_REQUEST);
  
  if(isset($cid)){
    $qry = "SELECT * FROM `blog_sub_inner_category` WHERE id = '$cid'";
  ...
  { 
    ...
    if(isset($cid) && isset($data))
      $qry = "UPDATE `blog_sub_inner_category` SET `si_name` = '$name', `i_id` = '$i_id', `s_id` = '$s_id', `c_id` = '$c_id', `seo_title` = '$seo_title', `seo_description` = '$seo_description', `seo_keywords` = '$seo_keywords', `seo_detail` = '$detail' WHERE id = '$cid'";
    else
      $qry = "INSERT INTO `blog_sub_inner_category` (`si_name`, `i_id`, `s_id`, `c_id`, `seo_title`, `seo_description`, `seo_keywords`, `seo_detail`) VALUES ('$name', '$i_id', '$s_id', '$c_id', '$seo_title', '$seo_description', '$seo_keywords', '$detail')";
          ...
          <div class="card ">
            ...
            <div class="row">
              ...
              <div class="col-md-6">
                ...
                <select class="form-control category-check" name="s_id" id="s_id" data-title="Inner category" data-url="<?= $base_url ?>get-inner" data-dependent="i_id" data-value="<?= isset($data['i_id']) ? $data['i_id'] : '' ?>" required></select>
              </div>
              <div class="col-md-6">
                <label>Inner Category</label>
                <select class="form-control" name="i_id" id="i_id" required></select>
              </div>
            	<div class="col-md-6">
      					<div class="form-group has-label">
      					<label>Sub Inner Category Name</label>
      					<input class="form-control" value="<?= isset($data['si_name']) ? $data['si_name'] : '' ?>" maxlength="100" name="name" type="text" required="true" placeholder="Enter Inner Category Name" />
      					</div>
      				</div>
                ...
                <div class="form-group has-label">
                  ...
                  <textarea class="form-control ckeditor" name="detail"><?= isset($data['seo_detail']) ? $data['seo_detail'] : '' ?></textarea>
		            ...
		            <div class="card-footer text-right">
		              ...
		              <a href="<?= $base_url ?>blog-category/sub_inner_list.php" class="btn btn-danger">Go to List</a>
		            </div>
	        	</div>
	        </div>
            </div>
